...
#include "eval-topology-bulk.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("EvaluationTopologyBulk");

NS_OBJECT_ENSURE_REGISTERED (EvaluationTopologyBulk);

TypeId
EvaluationTopologyBulk::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EvaluationTopologyBulk")
    .SetGroupName ("AqmEvaluationSuite")
  ;
  return tid;
}

EvaluationTopologyBulk::EvaluationTopologyBulk (std::string ScenarioName, uint32_t numFlows,
  ...
  : m_dumbbell (numFlows, p2pHelper, numFlows, p2pHelper, p2pHelper)
{
  ...
  m_queue->TraceConnectWithoutContext ("Enqueue", MakeCallback (&EvaluationTopologyBulk::PacketEnqueue, this));
  m_queue->TraceConnectWithoutContext ("Dequeue", MakeCallback (&EvaluationTopologyBulk::PacketDequeue, this));
  m_queue->TraceConnectWithoutContext ("Drop", MakeCallback (&EvaluationTopologyBulk::PacketDrop, this));
...
EvaluationTopologyBulk::~EvaluationTopologyBulk (void)
{
}

void
EvaluationTopologyBulk::DestroyConnection ()
{
  m_queue->TraceDisconnectWithoutContext ("Enqueue", MakeCallback (&EvaluationTopologyBulk::PacketEnqueue, this));
  m_queue->TraceDisconnectWithoutContext ("Dequeue", MakeCallback (&EvaluationTopologyBulk::PacketDequeue, this));
  m_queue->TraceDisconnectWithoutContext ("Drop", MakeCallback (&EvaluationTopologyBulk::PacketDrop, this));

  for (uint32_t i = 0; i < m_sinks.size (); i++)
    {
      m_sinks[i]->TraceDisconnectWithoutContext ("Rx", MakeCallback (&EvaluationTopologyBulk::PayloadSize, this));
    }
...
void
EvaluationTopologyBulk::PacketEnqueue (Ptr<const QueueDiscItem> item)
{
...
void
EvaluationTopologyBulk::PacketDequeue (Ptr<const QueueDiscItem> item)
{
...
void
EvaluationTopologyBulk::PacketDrop (Ptr<const QueueDiscItem> item)
{
  Ptr<const Ipv4QueueDiscItem> iqdi = Ptr<const Ipv4QueueDiscItem> (dynamic_cast<const Ipv4QueueDiscItem *> (PeekPointer (item)));
...
void
EvaluationTopologyBulk::PayloadSize (Ptr<const Packet> packet, const Address & address)
{
...
EvaluationTopologyBulk::CreateFlow (StringValue senderDelay, StringValue receiverDelay,
                                ...
                                DataRate rate, uint32_t initCwnd)
{
  ...
  if (transport_prot == "udp")
    {
      ...
      Ptr<EvalBulkApp> app = CreateObject<EvalBulkApp> ();
      app->Setup (ns3UdpSocket, remoteAddress.Get (), m_packetSize, maxPacket * m_packetSize);
      ...
      psink->TraceConnectWithoutContext ("Rx", MakeCallback (&EvaluationTopologyBulk::PayloadSize, this));
  ...
  Ptr<EvalBulkApp> app = CreateObject<EvalBulkApp> ();
  app->Setup (ns3TcpSocket, remoteAddress.Get (), m_packetSize, maxPacket * m_packetSize);
  m_dumbbell.GetLeft (m_flowsAdded - 1)->AddApplication (app);
